...
/** * @author  作者 E-mail: * @date 创建时间：2017年10月19日 下午5:38:22 * @version 1.0 * @parameter  * @since  * @return  */

@Controller
@RequestMapping("contentjh")
public class KFContentJhController {
	@Resource
	private KFContentJhService service;
	
	@RequestMapping("/creatcontentjh")
	public String creattask(Userinfo userinfo,HttpServletResponse response,HttpServletRequest request) throws Exception{
		...
		if(contentinfolist != null){
			...
			for (int i = 0; i < contentlist.length; i++) {
				...
				resultpp = service.insertcontentjh(realname, userID,new Date(), contentid, contentinfoid);
				if(resultpp >0){
					countcontent++;
				}
			}
	...
	@RequestMapping("/deletecontentjh")
	public String deletetask(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		resultpp = service.deletecontentjh(userID, contentid, contentinfoid);
		if(resultpp>0){
			JSONObject result=new JSONObject();
			result.put("success", true);
			ResponseUtil.write(response, result);	
